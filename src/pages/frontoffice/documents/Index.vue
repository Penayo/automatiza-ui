...
async function load() {
    loading.value = true;
    try {
        const params: DocumentListParams = { page: page.value, limit: limit.value };
    ...
    <div class="flex flex-col h-full gap-4 p-6">
        ...
        <div class="flex flex-wrap gap-3 items-end">
            <div class="flex flex-col gap-1">
                ...
                <InputText v-model="search" placeholder="Filename…" class="w-56" size="small" />
            </div>
            ...
            <div class="flex flex-col gap-1">
                ...
                <DatePicker v-model="dateFrom" date-format="yy-mm-dd" placeholder="Start date" class="w-40" size="small" show-button-bar />
            </div>

            <div class="flex flex-col gap-1">
                <label class="text-xs text-surface-500">To</label>
                <DatePicker v-model="dateTo" date-format="yy-mm-dd" placeholder="End date" class="w-40" size="small" show-button-bar />
            </div>

            <Button label="Clear" icon="pi pi-times" severity="secondary" size="small" text @click="clearFilters" />
        </div>

        <!-- Table -->
        <DataTable :value="docs" :loading="loading" size="small" striped-rows empty-message="No documents found." class="flex-1">
            ...
            <Column header="Source" style="width: 120px">
                <template #body="{ data }">
                    <Tag :value="sourceLabel(data.source)" :severity="sourceSeverity(data.source)" style="font-size: 0.7rem;" />
                </template>
            </Column>
        ...
        <Paginator :rows="limit" :total-records="total" :rows-per-page-options="[10, 20, 50]" @page="onPageChange" />
    </div>
</template>
